...
class EvaluationsGeneratorOperations {
     ...
     */
    void ajouterDetectionClicLigne(JTable evalsTable, JButton buttonSupp, JButton buttonModif) {

        evalsTable.getSelectionModel().addListSelectionListener(e -> {
            if (evalsTable.getSelectedRow() >= 0) {
                ...
                if (Globals.selectedStudent != null)
                    for (Evaluation eval : Globals.selectedStudent.getEvaluations()) {
                        if (eval.getNote() == note) {
                            if (eval.getEvalType().equals(typeString)) {
                                if (eval.getMat().getNom().equals(matString)) {
     ...
     * @param mainLabel
     */
    void afficherEvaluationsEleve(Eleve e, JTable evaluationsTable, JLabel mainLabel) {
     ...
     * @param champType       zone du type d'évaluation de la ligne sélectionné
     */
    void remplirChamps(int rowNumber, JTable evalsTable, JTextField champNote, JTextField champCorrecteur, JTextField champMatiere, JTextField champType) {
        ...
        Matiere mat = Matiere.trouverMatiere(matString, Globals.currentPromotion.getNom());
        if (mat == null) {
            mat = new Matiere(matString, Globals.currentPromotion.getNom());
        ...
        Evaluation eval = new Evaluation(note, mat, Globals.selectedStudent, prof);
     ...
     * @param gen
     */
    private void openWindowClicField(JTextField tfClique, TypeSelectionChoice typeFenetre, GenerateurEvaluations gen) {
     ...
     * @param gen
     */
    public void enableSubjectClicDetectionTF(JTextField matTF, GenerateurEvaluations gen) {

        System.out.println("activerDetectionClicMatiereTF()");
        openWindowClicField(matTF, TypeSelectionChoice.Matiere, gen);
    }
     ...
     * @param gen
     */
    void activerDetectionClicProfTF(JTextField correcteurTF, GenerateurEvaluations gen) {

        System.out.println("activerDetectionClicProfTF()");
        openWindowClicField(correcteurTF, TypeSelectionChoice.Professeur, gen);
    }
